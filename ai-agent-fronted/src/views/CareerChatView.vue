...
import { postSseText } from '../utils/sse'
...
onMounted(() => {
  ...
  // 进入页面自动生成 chatId（并给一个欢迎气泡，强化氛围）
  messages.value.push({
    id: `${Date.now()}-hello`,
    role: 'ai',
    content: '你好，我是AI职业规划师。把你的目标、现状和困惑发给我，我会逐步帮你拆解路线。',
  })
  scrollToBottom(false)
})
        ...
        <div ref="scrollerRef" class="chat">
          <div v-if="!hasChatted" class="empty">
            <div class="emptyCard">
              ...
              <div class="big">把你的目标发来</div>
              <div class="small">例如：目标岗位、当前水平、时间限制、最焦虑的点。我会边分析边流式输出建议。</div>
        ...
        <div class="composer">
          <div class="inputWrap">
            <textarea
              ...
              class="input"
              rows="1"
              placeholder="输入你的问题：目标岗位、当前技能、困惑点、时间规划……"
              :disabled="sending"
